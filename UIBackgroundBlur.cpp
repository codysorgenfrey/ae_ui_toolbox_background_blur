	...
	PF_LayerDef		*output )
{
	...
	PF_ADD_FLOAT_SLIDERX(	STR(StrID_Radius_Param_Name),
							BACKGROUNDBLUR_RADIUS_MIN,
							BACKGROUNDBLUR_RADIUS_MAX,
							BACKGROUNDBLUR_RADIUS_MIN,
							BACKGROUNDBLUR_RADIUS_MAX,
							BACKGROUNDBLUR_RADIUS_DFLT,
							PF_Precision_HUNDREDTHS,
							0,
							0,
							RADIUS_DISK_ID);
	
	out_data->num_params = BACKGROUNDBLUR_NUM_PARAMS;
    ...
    PF_LayerDef		*output )
{
    AEGP_SuiteHandler suites(in_data->pica_basicP);
    PF_Err err = PF_Err_NONE;
    PF_LayerDef *input = &params[BACKGROUNDBLUR_INPUT]->u.ld;
    
    if (params[BACKGROUNDBLUR_RADIUS]->u.fs_d.value){    // we're doing some blurring...
        ...
        //Blur the AEGP_World
        ERR(suites.WorldSuite3()->AEGP_FastBlur((A_FpLong)params[BACKGROUNDBLUR_RADIUS]->u.fs_d.value,
